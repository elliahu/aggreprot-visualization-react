...
interface IProps {
  onResidueClickedInStructure: (position: number) => void;
}

class MolstarViewer extends Component<IProps, IState> {
  viewer: any;
    ...
    this.viewer.plugin.behaviors.interaction.click.subscribe(
      (event: InteractivityManager.ClickEvent) => {
        ...
        if (residueIndex) {
          ...
          this.props.onResidueClickedInStructure(residueIndex.residueIndex);
        }
      }
    );
